...
export const CaseFilesModal: React.FC<CaseFilesModalProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 z-[60] flex items-center justify-center bg-[#0B0D10]/50 p-6"
      onMouseDown={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div
        className="w-full max-w-6xl h-[86vh] rounded bg-white border border-[#E4E7EC] flex flex-col overflow-hidden"
        role="dialog"
        aria-modal="true"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-[#EDEFF3] bg-[#FAFBFC]">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-white bg-[#2563EB] rounded px-3 py-1.5 hover:bg-[#1D4ED8] disabled:opacity-50"
            >
              {exporting ? (
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded text-[#69717D] hover:bg-[#EEF1F5]"
              aria-label="Close"
            >
              <X className="w-4 h-4" />
            </button>
          ...
          <div className="flex-1 min-h-0 flex">
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              <div className="flex items-center justify-between px-4 py-2 border-b border-[#EDEFF3] bg-white">
                ...
                <button
                  onClick={copy}
                  className="inline-flex items-center gap-1.5 text-[11px] font-medium text-[#69717D] hover:text-[#171A1F] border border-[#E1E4E8] rounded-sm px-2 py-1"
                >
                  {copied ? (
